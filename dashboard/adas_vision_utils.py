...
def annotate_bev(perc, ctrl, is_calibrating, auto_start_time):
    ...
    if perc.sl is not None:
        lx_safe = int(np.clip(np.polyval(perc.sl, yrow) + 130, 0, 639))
        cv2.line(dbg, (lx_safe, yrow-20), (lx_safe, yrow+20), (255, 0, 255), 2, cv2.LINE_AA)
        lbl(dbg, "130px", lx_safe+5, yrow-25, scale=0.35, color=(255, 0, 255))
    if perc.sr is not None:
        rx_safe = int(np.clip(np.polyval(perc.sr, yrow) - 100, 0, 639))
        cv2.line(dbg, (rx_safe, yrow-20), (rx_safe, yrow+20), (255, 0, 255), 2, cv2.LINE_AA)
        lbl(dbg, "100px", rx_safe-45, yrow-25, scale=0.35, color=(255, 0, 255))
    ...
    for yi in range(yrow-40,yrow+40,12): cv2.line(dbg,(tx,yi),(tx,yi+6),(0,255,255),2,cv2.LINE_AA)

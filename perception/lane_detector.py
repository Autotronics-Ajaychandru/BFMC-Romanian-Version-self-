...
class LaneDetector:
    ...
    def process(self, raw_frame, dt: float = 0.033, extra_offset_px=0.0,
        ...
        base_curv = self.tracker.get_curvature(320)
        y_eval = 180.0 + min(1.0, base_curv / 0.003) * 120.0 # Dynamic lookahead
        ...
        target_x, anchor = self.tracker.get_target_x(
            ...
            velocity_ms, last_steering, current_yaw, imu_yaw_rate=imu_yaw_rate
        )
        if not hasattr(self, "_target_ema"):
            self._target_ema = target_x
        
        # Make EMA heavily smooth small noise to prevent wobble, but track large changes fast
        delta = abs(target_x - self._target_ema)
        if delta > 20.0:
            alpha = 0.7 # Fast response for curves / corrections
        elif delta > 5.0:
            alpha = 0.3 # Moderate smoothing
        else:
            alpha = 0.05 # Heavy damping on straight lines for micro-noise
        ...
        # Smooth heading heavily to prevent reactive wobble

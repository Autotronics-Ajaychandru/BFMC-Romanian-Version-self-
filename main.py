...
try:
    ...
    class IMUSensor:
        ...
        def start(self): pass
        def stop(self): pass
        def get_roll(self): return 0.0
        def get_pitch(self): return 0.0
        def get_yaw(self): return 0.0
        def get_has_hardware(self): return self.has_hardware
...
class BFMC_App:
    ...
    def _estimate_upcoming_curve(self, min_angle_deg: float = 25.0):
        ...
        for i in range(len(self.path) - 1):
            n1 = str(self.path[i]); n2 = str(self.path[i+1])
        ...
        next_index = min(current_index + 1, len(self.path) - 2)
        if next_index <= current_index:
            return "STRAIGHT"

        def heading(p_src, p_dst):
            return math.atan2(p_dst[1] - p_src[1], p_dst[0] - p_src[0])

        n1 = str(self.path[current_index])
        n2 = str(self.path[current_index + 1])
        n3 = str(self.path[next_index + 1])
        if n1 not in self.map_engine.G.nodes or n2 not in self.map_engine.G.nodes or n3 not in self.map_engine.G.nodes:
            return "STRAIGHT"

        p1 = (float(self.map_engine.G.nodes[n1].get('x', 0)), float(self.map_engine.G.nodes[n1].get('y', 0)))
        p2 = (float(self.map_engine.G.nodes[n2].get('x', 0)), float(self.map_engine.G.nodes[n2].get('y', 0)))
        p3 = (float(self.map_engine.G.nodes[n3].get('x', 0)), float(self.map_engine.G.nodes[n3].get('y', 0)))

        angle1 = heading(p1, p2)
        angle2 = heading(p2, p3)
        turn_deg = math.degrees((angle2 - angle1 + math.pi) % (2 * math.pi) - math.pi)

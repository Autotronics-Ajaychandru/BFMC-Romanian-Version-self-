...
class StanleyController:
    ...
    def compute(self, target_x_px: float, heading_rad: float,
        ...
        err_px = target_x_px - 320.0
        
        # Deadband to eliminate micro-oscillations (wobble) on straight lines
        if abs(map_curvature) < 0.001 and abs(err_px) < 12.0:
            err_px = 0.0
            
        ce_m = err_px / ppm

        # Dynamic gain: more aggressive in curves, smooth on straights
        dyn_k = self.k + (1.0 if abs(map_curvature) > 0.0015 else 0.0)
        k_eff = dyn_k * min(1.0, velocity_ms / 0.25) if velocity_ms > 0 else dyn_k
...
class Controller:
    ...
    def __init__(self):
        ...
        self.stanley    = StanleyController(k=2.5, ks=0.5, wheelbase_m=0.23)
    ...
    def compute(self, perc_res,
        ...
        visual_curv_m = getattr(perc_res, 'signed_curvature', 0.0) * (max(perc_res.lane_width_px, 50) / 0.35)
        curv_to_use = map_curvature if map_curvature != 0.0 else visual_curv_m
        ...
        raw_steer, react_steer_deg, ff_steer_deg = self.stanley.compute(
            ...
            map_curvature=curv_to_use)
            
        # IMU yaw rate damping
        imu_yaw_rate = getattr(perc_res, 'imu_yaw_rate', 0.0)
        kd_yaw = 0.45  # degrees of steer per degree/sec of yaw rate (increased for damping)
        raw_steer -= kd_yaw * imu_yaw_rate
        ...
        # Dynamic steering filter: Heavy smoothing on straights to prevent wobble, fast response on curves
        is_straight = abs(curv_to_use) < 0.001
        alpha = 0.65 if is_straight else 0.2
        ...
        return ControlOutput(
            ...
            lookahead_px    = 520.0 - perc_res.y_eval,
